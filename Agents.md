...
- **Geschlossene Parks & Attraktionsdaten:** Öffnungszeiten sind auch für aktuelle Attraktionsdaten maßgeblich. Wenn ein Park laut Öffnungszeiten nicht aktuell geöffnet ist, müssen lokale Wartezeiten für diesen Park entfernt bzw. nicht aus öffentlichen Snapshots übernommen werden, damit keine alten Attraktionsdaten aus vergangenen Öffnungstagen als aktuelle Daten erscheinen.
...
- **Watchlist-Benachrichtigungen:** Watchlist-Alarme laufen rein lokal über WorkManager (`NotificationWorker`). Neue Alarme starten zusätzlich einen schnellen Einmal-Check; periodische Hintergrundchecks laufen alle 30 Minuten und werden nach Neustart/App-Update erneut registriert. Es gibt bewusst keine Firebase-/FCM-Abhängigkeit und keine Worker-Secrets. Android kann lokale periodische Arbeit im Doze/Standby verzögern; das ist als Einschränkung akzeptiert.
- **Zentrale Statistik-Snapshots:** Der Cloudflare Worker sammelt App-Daten per Cron in drei versetzten 5-Minuten-Shards. Für den KV-Free-Plan bleibt die Anzahl der Writes niedrig: ein kompakter Tages-Shard pro Shard-Lauf, der Statistik-Index nur etwa stündlich oder bei neuen Park-/Datums-Kombinationen; `latest.json` und `trend-history.json` werden im Shard-Cron nicht geschrieben. Im Shard-Modus werden für Statistiken nur Öffnungszeiten und Wartezeiten abgefragt, nicht Crowd-Level. Globale Marker sind über `/app-data/global-markers/latest.json` abrufbar und werden ohne zusätzliche KV-Writes aus den aktuellen Tages-Shards abgeleitet. Mehr History-Shards erhöhen die KV-Writes; sehr viele Parks können zusätzlich Worker-Subrequest-, API- und KV-Größenlimits sprengen.
