...
- **In-App-Update-Installation:** Der Update-Screen (`UpdateBanner.kt`/`UpdateViewModel.kt`) lädt die APK seit der In-App-Download-Funktion selbst über `update/ApkDownloader.kt` herunter (OkHttp, Fortschrittsanzeige) statt nur einen Browser-Intent auf `apkUrl` zu öffnen. Ist in `release.json` ein `sha256` hinterlegt, wird die heruntergeladene Datei vor der Installation geprüft und bei Abweichung verworfen. Die Installation selbst läuft über `update/ApkInstaller.kt`: fehlt die Berechtigung `REQUEST_INSTALL_PACKAGES`/"Installation aus unbekannten Quellen", öffnet ein Tap zunächst die passende Systemeinstellung; ein erneuter Tap startet danach den eigentlichen System-Installationsdialog. Ein vollständig klickloses Auto-Update ist außerhalb des Play Store android-seitig nicht möglich – der "Release"-Button bleibt als Browser-Fallback erhalten, falls der In-App-Download fehlschlägt.
